...
// Frame re-renders on every page change; these two layers never change, so skip re-rendering them.
const StaticAtmosphere = memo(Atmosphere);
const StaticEmojiLanyard = memo(EmojiLanyard);

// Fetch/decode an image ahead of time (no visual effect) so swiping to it doesn't wait on the network.
const preloaded = new Set();
const preload = src => { if (preloaded.has(src)) return; preloaded.add(src); const img = new Image(); img.src = src; img.decode?.().catch(() => { }) };

function Frame({ children }) {
  const { stepIndex, sections, currentSection, goToStep, isFirst, isLast } = useJourney();
  return <main className="universe">
    ...
    {currentSection.id === "final" && <button className="center-button neo-button" onClick={() => goToStep("funny-archive")} aria-label="Go to page 12">Etha Click panudi oru fun Eruku 🤣</button>}
...
function Welcome() {
  ...
  useEffect(() => { if (!ref.current) return; const tween = gsap.fromTo(ref.current.querySelectorAll(".hero-word"), { y: 70, opacity: 0, rotate: 3 }, { y: 0, opacity: 1, rotate: 0, duration: 1.1, stagger: .08, ease: "power4.out" }); return () => tween.kill() }, []);
  ...
  return <Stage id="love-gate" photo={C.photos[1]} position="center">
    ...
    <div className="question-card neo-card">
      ...
      <div ref={area} className="choice-zone">
        ...
        <motion.button className="choice choice--no neo-button neo-button--white" animate={{ x: noPos.x, y: noPos.y }} transition={{ type: "spring", duration: .13, bounce: 0 }} onPointerEnter={dodge} onFocus={() => setReaction("cry")} onClick={dodge}>{C.loveGate.no}</motion.button>
      </div>
...
function Flower() {
  const { goNext } = useJourney(); const [bloom, setBloom] = useState(false); const ref = useRef(null); const tlRef = useRef(null); useEffect(() => () => { tlRef.current?.kill() }, []);
  const start = () => { if (bloom) return; setBloom(true); if (ref.current) { const tl = tlRef.current = gsap.timeline(); tl.fromTo(ref.current.querySelector(".stem"), { scaleY: 0 }, { scaleY: 1, duration: .8, ease: "power2.out" }).fromTo(ref.current.querySelectorAll(".petal"), { scale: 0, opacity: 0 }, { scale: 1, opacity: 1, duration: .55, stagger: .08, ease: "back.out(2)" }, "-=.35").fromTo(ref.current.querySelector(".flower-core"), { scale: 0 }, { scale: 1, duration: .4, ease: "back.out(3)" }, "-=.2") } };
...
function Photos() {
  ...
  useEffect(() => { [1, 2, -1].forEach(o => preload(GIRLFRIEND_PHOTOS[(active + o + GIRLFRIEND_PHOTOS.length) % GIRLFRIEND_PHOTOS.length].src)) }, [active]);
...
function FunnyArchive() {
  ...
  const funnyMedia = [
    ...
    { type: "video", src: "/assets/funny-videos/funny-video-02.mp4", caption: "This is why I love our chaos." },
    { type: "video", src: "/assets/funny-videos/funny-video-03.mp4", caption: "This is why I love our chaos." },
    { type: "video", src: "/assets/funny-videos/funny-video-04.mp4", caption: "A moment worth replaying. 😂" },
  ];
  const item = funnyMedia[active];
  const next = dir => setActive(i => (i + dir + funnyMedia.length) % funnyMedia.length);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { [1, 2, -1].forEach(o => { const m = funnyMedia[(active + o + funnyMedia.length) % funnyMedia.length]; if (m.type === "photo") preload(m.src) }) }, [active]);
  ...
  return <Stage id="funny-archive" photo={C.photos[11]} position="center">
    ...
    <MagneticButton onClick={goNext}>END →</MagneticButton>
  </Stage>
}
